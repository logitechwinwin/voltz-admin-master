...
export const volunteerTableRow = ({ data, role, type }) => {
    ...
    let updatedData = {
        ...
        volunteerName: deal ? sourceWallet?.user?.firstName + " " + targetWallet?.user?.lastName : targetWallet?.user?.firstName + " " + targetWallet?.user?.lastName,
        volunteerImage: deal ? sourceWallet?.user?.profileImage : targetWallet?.user?.profileImage,
        type: data?.type,
    }
    return (
        <TableRow>
            ...
            <TableCell sx={{ textAlign: 'center' }}>
                {updatedData?.eventName ? <Stack direction="row" alignItems="center" gap={1}>
