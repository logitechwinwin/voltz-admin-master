  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await ApiManager({
        ...
        path: isUpdating ? `campaign-manager/${managerId}` : `campaign-manager`,
        params: {
      ...
      router.push("/ngo/campaign-managers");
    } catch (error) {
  ...
  return (
    <Container maxWidth="lg" sx={styles.table}>
      ...
      <Typography variant="h4" component="h1" fontWeight="bold" color={"text.heading"} mb={2}>
        {isUpdating ? `Update Campaign Manager` : `Add Campaign Manager`}
      </Typography>
      ...
      <Stack component="form" onSubmit={handleSubmit}>
        ...
        <Grid container spacing={2} marginY={3} mt={"110px"}>
          ...
          <Grid item sm={6} xs={12}>
            <InputField required label="Email" error={formErrors?.email} name="email" value={formData.email} onChange={handleChange} />
          </Grid>
        ...
        <Grid container spacing={2} mt={4} justifyContent="center">
          ...
          <Grid item lg={3} md={4} sm={6} xs={12}>
            <PrimaryButton type="submit">{isUpdating ? `Update` : `Submit`}</PrimaryButton>
          </Grid>
        </Grid>
      </Stack>
    </Container>
...
export default AddCampaignManager;
