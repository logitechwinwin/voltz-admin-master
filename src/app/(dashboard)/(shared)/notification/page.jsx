...
import { handleNavigation } from "@/constant";
...
const Notification = ({ loading = true,user, notificationData = {}, read = false, setNotification, getAllNotifications }) => {
  const router = useRouter();
  ...
  const handleNotificationClick = async () => {
    router.push(handleNavigation(notificationData,user?.role));
    if (notificationData.status === "sent") {
      ...
      setNotification(prev=>(
        prev.map(item=>item?.id === notificationData?.id ? {...item, status:"read"} : item)
      ))
      } catch (error) {
