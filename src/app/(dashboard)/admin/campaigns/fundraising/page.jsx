...
const Charities = () => {
  ...
  const fetchCharity = async () => {
    setLoading(true);
    try {
      let path = `events?type=charity&page=${page + 1}&perPage=${itemsPerPage}`;
                ...
                charities?.items?.map((charity) => (
                  <TableRow key={charity.id}>
                    ...
                    <TableCell>
                      <Stack direction="row" alignItems="center" spacing={1}>
                        <Avatar
                          src={charity.user.profileImage}
                          alt={charity.user.name}
                        />
                        <Typography whiteSpace="nowrap" variant="body2">
                          {charity.user.name}
                        </Typography>
                    ...
                    <TableCell>
                      ...
                      >
                        <Switch
                          checked={charity?.activationStatus === "active"}
                          onChange={() => handleToggleStatus(charity)}
                          color="primary"
                        />
                      </Tooltip>
                    </TableCell>
                    <TableCell sx={{ textAlign: "center" }}>
                      ...
                      >
                        <Tooltip title="View" arrow>
                          <IconButton
                            onClick={() =>
                              router.push(`/admin/campaigns/${charity.id}`)
                            }
                          >
                            <VisibilityIcon />
                          </IconButton>
